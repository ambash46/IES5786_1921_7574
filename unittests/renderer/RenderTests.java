...
class RenderTests {
    ...
    /**
     * Renders a heart shape loaded from {@code scenes/heart.xml}.
     * Scene: two spheres (top lobes) + one triangle (bottom tip),
     * red ambient light, white background.
     */
    @Test
    void testHeartRender() {
        renderSceneXML(baseCameraBuilder(), "heart") //
                .writeToImage("heart render test");
    }
    ...
    Camera renderSceneXML(Camera.Builder builder, String xmlName) {
        Scene scene = new XmlSceneLoader().load(xmlName);
        return builder //
                .setRayTracer(scene, RayTracerType.SIMPLE) //
                .build() //
                .renderImage(); //
     ...
     */
    @Disabled("JSON support removed")
    @Test
    void testBasicRenderJson() { /* JSON support removed */ }
}
