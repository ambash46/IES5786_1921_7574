 ...
 * @author Dan
 */
@SuppressWarnings({"java:S109", "SpellCheckingInspection"})
class RenderStage6Tests {
    ...
    * bodies and render it into a png image with a grid
    */
   /** Renders the emission-color scene from an XML file (bonus). */
   @Test
   void testRenderEmissionColorXml() {
      createImage(new XmlSceneLoader().load("emissionRenderTest"), "emission render test xml");
   }

   /** Renders the ambient-color scene from an XML file (bonus). */
   @Test
   void testRenderAmbientColorXml() {
      createImage(new XmlSceneLoader().load("ambientRenderTest"), "ambient render test xml");
   }
